...
{
	public:
		ElasticState(Eigen::Vector3d, Eigen::Matrix3d, double, double, double);
		ElasticState();
		~ElasticState();	
		//velocity, Deformation grad, Density, Entropy 
		ElasticState F();
		
		void invariants(Eigen::Matrix3d);
		...
		void initialStates(std::ostream& os);

		void prepareOutputs();

		double u_(int);

		double sigma_(int, int); //returns components of the stress tensor

		double mod_u(); //length squared

		double getInvariant(int);

		static void checkEOSConstants();
		...
		ElasticState operator*(double);

		friend ElasticState operator*(double, ElasticState);	

		ElasticState operator/(ElasticState);

		/* ElasticState operator=(ElasticState); */	

		friend std::ostream& operator<<(std::ostream&, ElasticState&);
	...
	private:
		...
		static const double alpha = 1.001, beta = 3.001, gamma = 2.001, b_0 = 2100, c_0 = 4600, c_v = 390, T_0 = 300, B_0, K_0;

		Eigen::Vector3d u_0;

		Eigen::Matrix3d F_0, sigma;

		double rho_0, E_0;

		double rho, e, E, S;

		double I[6]; //--invariants

		Eigen::VectorXd v;

		void dInvariants_G();

		void stress(Eigen::Matrix3d);

		void stressi(Eigen::Matrix3d);

		double kron(int i, int j);

		Eigen::Matrix3d strainTensor(Eigen::Matrix3d);

};
/* const double ElasticState::B_0 = b_0*b_0; */
/* const double ElasticState::K_0 = c_0*c_0-(4./3.)*B_0; */
/* const string ElasticState::name[] = {"rhou_1", "rhou_2", "rhou_3", "rhoF_11", "rhoF_12", "rhoF_13", "rhoF_21", "rhoF_22", "rhoF_23", "rho_31", "rho_32", "rho_33", "rhoE"}; */
/* //1. Calc U, */
/* // */
